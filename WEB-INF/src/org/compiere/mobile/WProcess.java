...
public class WProcess extends HttpServlet
{
		...
		throws ServletException, IOException
	{
		...
		if (AD_Process_ID == 0)
		{
			MobileUtil.createErrorPage(request, response, this, "No Process");
			return;
		}
	 ...
	 *	@return Page
	 */
	private MobileDoc createParameterPage (MobileSessionCtx wsc, int processId, int windowID,int tableID,int recordID,int Type,
...
//Modified by Rob klein 4/29/07	
		...
		int AD_PInstance_ID, File file)
	{
		...
		//	OK
		return MobileUtil.streamFile(response, file);
	}	//	streamResult
