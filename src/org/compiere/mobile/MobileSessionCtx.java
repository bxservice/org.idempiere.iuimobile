...
public class MobileSessionCtx implements Serializable
{
	 ...
	 *	@return ctx or null
	 */
	public static MobileSessionCtx get (HttpServletRequest request, int W_Store_ID)
	{
		...
		if (session == null)
			return null;
		MobileSessionCtx wsc = (MobileSessionCtx)session.getAttribute(NAME);
		//	Create New
		if (wsc == null)
		{
			wsc = new MobileSessionCtx(request, W_Store_ID);
	 ...
	 *	@return ctx or null
	 */
	public static MobileSessionCtx get (HttpServletRequest request)
	{
		HttpSession session = request.getSession(false);
		if (session == null)
			session = request.getSession(true);
		...
		if (wsc == null)
		{
			wsc = new MobileSessionCtx (request);
			session.setAttribute(NAME, wsc);
		}
	...
	/**	Logger							*/
	static private CLogger			log = CLogger.getCLogger (MobileSessionCtx.class);
	/** Cache 60 minutes				*/
	 ...
	 * 	@param request request
	 */
	private MobileSessionCtx (HttpServletRequest request)
	{
	 ...
	 * 	@param request request
	 */
	private MobileSessionCtx (HttpServletRequest request, int W_Store_ID)
	{
		log.info (request.getContextPath() + " (" + request.getRemoteAddr() 
			+ " - " + request.getLocale() + ") #" + counter);
		ctx = new Properties();
	...
	private void setWStore (String contextPath)
	{
		...
		if ("/mobile".equals(contextPath))	//	HTML UI
			return;
	...
	private void setLanguage (HttpServletRequest request)
	{
		...
		if (mobileLanguage != null) AD_Language = mobileLanguage;
		if (AD_Language == null)
		{
			//  Check Cookie
