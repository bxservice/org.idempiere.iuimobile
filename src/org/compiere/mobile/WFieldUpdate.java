...
public class WFieldUpdate extends HttpServlet
{
		...
		throws ServletException, IOException
	{
		...
		//  Get Parameter
		String formName = MobileUtil.getParameter (request, FIELD_FORM);
		String fieldName = MobileUtil.getParameter (request, FIELD_NAME);
		String fieldValue = MobileUtil.getParameter (request, FIELD_VALUE);
		String locationValue = MobileUtil.getParameter (request, LOCATION_VALUE);
		
		log.info("doPost - Form=" + formName + " - Field=" + fieldName + " - Value=" + fieldValue+ " - Location=" + locationValue);

		//  Document
		MobileDoc doc = createPage (wsc, ws, formName, fieldName, fieldValue, locationValue);
	 ...
	 *  @param fieldValue
	 */
	private static MobileDoc createPage (MobileSessionCtx wsc, WWindowStatus ws,
	 ...
	 *  @param fieldValue
	 */
	private static void reply_Login2_Role (body body, MobileSessionCtx wsc, 
