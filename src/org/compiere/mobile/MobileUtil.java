...
public final class MobileUtil
{
		...
		HttpServlet servlet, String message) throws ServletException, IOException
	{
		...
		MobileDoc doc = MobileDoc.create (windowTitle);
		...
		throws ServletException, IOException
	{
		...
		MobileDoc doc = MobileDoc.createPopup(windowTitle);
		...
		//	fini
		createResponseFragment(request, response, servlet, null, doc);
	}   //  createErrorPage
	 ...
	 *  <p>
	 *  - End Session
	 *  - Go to start page (e.g. /mobile/index.html)
	 *
	 *  @param request request
	 *  @param response response
	 *  @param servlet servlet
		...
		HttpServlet servlet, Properties ctx, String AD_Message) throws ServletException, IOException
	{
		...
		String url = MobileEnv.getBaseDirectory("WLogin");
		//
		MobileDoc doc = null;
		if (ctx != null && AD_Message != null && !AD_Message.equals(""))
			doc = MobileDoc.create (Msg.getMsg(ctx, AD_Message));
		else if (AD_Message != null)
			doc = MobileDoc.create (AD_Message);
		else
			doc = MobileDoc.create (false);
	...
	public static input getLoginButton (Properties ctx)
	{
		...
		cmd.append(MobileEnv.getBaseDirectory("index.html"));
		cmd.append("');");
	...
	public static String getParameter (HttpServletRequest request, String parameter)
	{
		...
		//	Convert
		if (enc != null && !MobileEnv.ENCODING.equals(enc))
		{
			try
			{
				String dataEnc = new String(data.getBytes(enc), MobileEnv.ENCODING);
				log.log(Level.FINER, "Convert " + data + " (" + enc + ")-> " 
					+ dataEnc + " (" + MobileEnv.ENCODING + ")");
				data = dataEnc;
			}
			catch (Exception e)
			{
				log.log(Level.SEVERE, "Convert " + data + " (" + enc + ")->" + MobileEnv.ENCODING);
			}
		}
    ...
    {
        String value = MobileUtil.getParameter(request, parameter);
        if(value == null) 
        	return value;
        if (value.length() == 0) 
        	return null;
    ...
    public static void reload(String logMessage, String jsp, HttpSession session, HttpServletRequest request, HttpServletResponse response, ServletContext thisContext)
            throws ServletException, IOException
    {
        session.setAttribute(MobileSessionCtx.HDR_MESSAGE, logMessage);
	...
	public static void createResponse (HttpServletRequest request, HttpServletResponse response,
		HttpServlet servlet, Properties cookieProperties, MobileDoc doc, boolean debug) throws IOException
	{
		...
		//	doc.output(System.out);
			MobileEnv.addFooter(request, response, servlet, doc.getBody());
		//	doc.output(System.out);
		}
	 ...
	 *  @throws IOException
	 */
	public static void createResponseFragment (HttpServletRequest request, HttpServletResponse response,
		HttpServlet servlet, Properties cookieProperties, MobileDoc doc) throws IOException
	{
		response.setHeader("Cache-Control", "no-cache");
		response.setContentType("text/html; charset=UTF-8");
	...
	{
		response.setContentType("text/html; charset=UTF-8");
		MobileDoc doc = MobileDoc.create(title);
		body b = doc.getBody();
	...
	//	System.out.println("String=" + data);
		try
		{
			result = URLDecoder.decode(data, MobileEnv.ENCODING);
		}
		catch (UnsupportedEncodingException e)
		{
			log.log(Level.SEVERE, "decode" + MobileEnv.ENCODING, e);
			String enc = System.getProperty("file.encoding");      //  Windows default is Cp1252
			try
			{
	...
	public static String streamFile (HttpServletResponse response, File file)
	{
		...
		try
		{
			int bufferSize = 8*1024; //	8k Buffer
	...
	public static boolean updateFields (HttpServletRequest request, WebUser wu, boolean updateEMailPwd)
	{
		if (updateEMailPwd)
		{
			String s = MobileUtil.getParameter (request, "PasswordNew");
			...
			//
			s = MobileUtil.getParameter (request, "EMail");
			if (!MobileUtil.isEmailValid (s))
			{
		...
		String s = MobileUtil.getParameter (request, "Name");
		if (s != null && s.length() != 0)
			wu.setName(s.trim());
		else
			mandatory.append(" - Name");
		s = MobileUtil.getParameter (request, "Company");
		if (s != null && s.length() != 0)
			wu.setCompany(s);
		s = MobileUtil.getParameter (request, "Title");
		if (s != null && s.length() != 0)
			wu.setTitle(s);
		//
		s = MobileUtil.getParameter (request, "Address");
		if (s != null && s.length() != 0)
			wu.setAddress(s);
		else
			mandatory.append(" - Address");
		s = MobileUtil.getParameter (request, "Address2");
		if (s != null && s.length() != 0)
			wu.setAddress2(s);
		//
		s = MobileUtil.getParameter (request, "City");
		if (s != null && s.length() != 0)
			wu.setCity(s);
		else
			mandatory.append(" - City");
		s = MobileUtil.getParameter (request, "Postal");
		if (s != null && s.length() != 0)
			wu.setPostal(s);
		else
			mandatory.append(" - Postal");
		//	Set Country before Region for validation
		s = MobileUtil.getParameter (request, "C_Country_ID");
		if (s != null && s.length() != 0)
			wu.setC_Country_ID(s);
		s = MobileUtil.getParameter (request, "C_Region_ID");
		if (s != null && s.length() != 0)
			wu.setC_Region_ID(s);
		s = MobileUtil.getParameter (request, "RegionName");
		if (s != null && s.length() != 0)
			wu.setRegionName(s);
		//
		s = MobileUtil.getParameter (request, "Phone");
		if (s != null && s.length() != 0)
			wu.setPhone(s);
		s = MobileUtil.getParameter (request, "Phone2");
		if (s != null && s.length() != 0)
			wu.setPhone2(s);
		s = MobileUtil.getParameter (request, "C_BP_Group_ID");
		if (s != null && s.length() != 0)
			wu.setC_BP_Group_ID (s);
		s = MobileUtil.getParameter (request, "Fax");
		if (s != null && s.length() != 0)
			wu.setFax(s);
		//
